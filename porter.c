...
PORTS* key_port_starter() {
    ...
    p* kp = malloc(sizeof(struct PORT_STRUCT));
    ...
    kp->dim = calloc(4,sizeof(struct DIMENSIONS));
    for(int i = 0; i < 4; i++) {
...
PORTS* key_port_setup_dim(PORTS* port) {
    /* Setting this up for dimensions.c */
    port->dim_ = malloc(sizeof(struct DIM_STRUCT*)*sizeof(struct PORT_STRUCT*)); // We want a simple block of memory
